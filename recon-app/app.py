...
@app.route("/debug/praxis")
@require_admin_auth
def debug_praxis():
    """Diagnostic endpoint — tests Praxis DB connectivity with full error detail."""
    import socket, os, struct, time

    host = os.environ.get("PRAXIS_HOST", "?")
    port = int(os.environ.get("PRAXIS_PORT", 5433))
    results = {}

    # 1. DNS resolution
    try:
        resolved = socket.gethostbyname(host)
        results["dns"] = f"OK — {host} → {resolved}"
    except Exception as e:
    ...
    try:
        ...
        # Read first bytes PostgreSQL would send (SSL request / auth challenge)
        s.settimeout(3)
        try:
            first_bytes = s.recv(16)
            results["tcp"] = f"OK in {ms}ms — first bytes: {first_bytes.hex() or '(none)'}"
        except socket.timeout:
            results["tcp"] = f"OK in {ms}ms — connected but server sent no bytes in 3s"
        s.close()
    except Exception as e:
        results["tcp"] = f"FAIL — {e}"

    # 3. Also test direct to 161.97.162.143:5433 regardless of env
    try:
        t0 = time.time()
        s2 = socket.create_connection(("161.97.162.143", 5433), timeout=5)
        ms2 = round((time.time() - t0) * 1000)
        s2.settimeout(3)
        try:
            b2 = s2.recv(16)
            results["direct_tcp"] = f"OK in {ms2}ms — first bytes: {b2.hex() or '(none)'}"
        except socket.timeout:
            results["direct_tcp"] = f"OK in {ms2}ms — connected, server silent"
        s2.close()
    except Exception as e:
        results["direct_tcp"] = f"FAIL — {e}"

    # 4. psycopg2 with current config
    try:
        import psycopg2
        conn = psycopg2.connect(
            host=host, port=port,
            dbname=os.environ.get("PRAXIS_DB","operations"),
            user=os.environ.get("PRAXIS_USER","praxis_reader"),
            password=os.environ.get("PRAXIS_PASS",""),
            connect_timeout=5, sslmode="disable",
        )
        cur = conn.cursor()
        cur.execute("SELECT COUNT(*) FROM praxis_transactions")
        n = cur.fetchone()[0]
        conn.close()
        results["psycopg2"] = f"OK — praxis_transactions has {n:,} rows"
    except Exception as e:
        results["psycopg2"] = f"FAIL — {e}"

    # 5. Environment
    results["env"] = {
        "PRAXIS_HOST": host, "PRAXIS_PORT": port,
        "PRAXIS_DB": os.environ.get("PRAXIS_DB","?"),
        "PRAXIS_USER": os.environ.get("PRAXIS_USER","?"),
    }
