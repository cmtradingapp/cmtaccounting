...
# The Signals "Boxes" view renders a trade-anatomy panel purely from each
# signal's own fields (entry/SL/TP/direction/outcome/close_price/duration),
# which the /signals/api/list payload already carries — so there is no chart
# endpoint here. An earlier version bucketed dealio.ticks into hourly candles,
# but that feed is a 30-min snapshot: far too coarse to render an M1/M5 trade
# that resolves in seconds, so the candles couldn't show the trade at all.
