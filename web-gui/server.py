...
def upload_files():
    ...
    # Deduplicate columns across all bank files
...
def get_rates():
    ...
    # These 28 fiat currencies were extracted from 83 historical Rates*.xlsx files
...
def reconcile():
    ...
    try:
        # 1. Find all uploaded bank files and the CRM file
        bank_paths = []
        crm_path = None
        ...
        # 2. Parse and concatenate ALL Bank/PSP files
        bank_frames = []
        ...
        # 4. Auto-detect join keys via column name heuristics
        bank_ref_col = None
        crm_ref_col = None
        
        # Bank side: look for reference/transaction ID columns
        ...
        # CRM side: look for psp_transaction_id (the proven join key from our investigation)
        ...
        if bank_ref_col and crm_ref_col:
            ...
            bank_df['_join_key'] = bank_df[bank_ref_col].astype(str).str.strip()
            crm_df['_join_key'] = crm_df[crm_ref_col].astype(str).str.strip()
            
            # Left join: all CRM rows, matched with bank where possible
            merged = crm_df.merge(bank_df, on='_join_key', how='outer', suffixes=('_crm', '_bank'), indicator=True)
            ...
            # Calculate fee discrepancies if amount columns found
            unrecon_fees = 0.0
